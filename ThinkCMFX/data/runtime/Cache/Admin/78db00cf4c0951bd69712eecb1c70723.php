...
<div class="wrap js-check-wrap">
    ...
    <form class="well form-search" method="get" action="/junwei/index.php?s=/Admin/Feedback/show">
        反馈内容：
        <input type="text" name="keyword" style="width: 200px;" value="<?php echo ($keyword); ?>"
               placeholder="请输入关键字...">
        ...
        <table class="table table-hover table-bordered table-list">
            ...
            <?php if(is_array($data)): foreach($data as $key=>$vo): ?><tr>
                    <td><input type="checkbox" class="js-check" data-yid="js-check-y" data-xid="js-check-x"
                               name="ids[]" value="<?php echo ($vo["f_id"]); ?>"></td>
                    <td><a><?php echo ($vo["f_id"]); ?></a></td>
                    <td><?php echo ($vo["f_desc"]); ?></td>
                    <td><?php echo ($vo["user_login"]); ?></td>
                    <td><?php echo ($vo["f_time"]); ?></td>
                    <td><?php echo ($vo["f_information"]); ?></td>
                    <td>
                        <a href="<?php echo U('Feedback/feeddel',array('id'=>$vo['f_id']));?>" class="js-ajax-delete"><?php echo L('DELETE');?></a>
                    </td>
                </tr><?php endforeach; endif; ?>

        </table>
        <div class="table-actions">
            <button class="btn btn-danger btn-small js-ajax-submit" type="submit" data-action="<?php echo U('Feedback/feeddel');?>" data-subcheck="true" data-msg="你确定删除吗？"><?php echo L('DELETE');?></button>
        </div>
        <div class="pagination" id="pageBar"><?php echo ($page); ?></div>
    </form>
</div>
